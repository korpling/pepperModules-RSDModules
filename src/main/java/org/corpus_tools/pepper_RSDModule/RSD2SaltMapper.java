...
public class RSD2SaltMapper extends PepperMapperImpl {
  ...
  public DOCUMENT_STATUS mapSDocument() {
    ...
    // assign customization values from importer properties
    this.namespace = (String) getProperties().getProperties()
        .getProperty(RSDImporterProperties.NAMESPACE, "rsd");
    ...
    this.edgeType =
        (String) getProperties().getProperties().getProperty(RSDImporterProperties.EDGETYPE, "rsd");
    this.edgeAnnoName = (String) getProperties().getProperties()
        .getProperty(RSDImporterProperties.EDGEANNONAME, "func");
    if (getProperties().getProperty(RSDImporterProperties.ADDEDUNUM)
        .getValue() instanceof Boolean) {
      this.addEduNum =
          (Boolean) getProperties().getProperty(RSDImporterProperties.ADDEDUNUM).getValue();
    } else {
      this.addEduNum = Boolean.valueOf(
          (String) getProperties().getProperty(RSDImporterProperties.ADDEDUNUM).getValue());
    }
    if (getProperties().getProperty(RSDImporterProperties.ADDRELANNO)
        .getValue() instanceof Boolean) {
      this.addRel =
          (Boolean) getProperties().getProperty(RSDImporterProperties.ADDRELANNO).getValue();
    } else {
      this.addRel = Boolean.valueOf(
          (String) getProperties().getProperty(RSDImporterProperties.ADDRELANNO).getValue());
    }
    ...
    try {
      ...
      String errorMessage = "Input file could not be read. Aborting conversion of file "
          + this.getResourceURI() + ".";
    ...
    String tokID;
    String textOffset; // TODO: properly decode text offsets to reconstruct whitespace preserving
                       // STextualDS
    String eduText;
    String eduID;
    String annoVal;
    ...
    for (int rowIndex = 0; rowIndex < numOfTuples; rowIndex++) {
      try {
        ...
        String errorMessage =
            String.format("line %d of input file could not be read. Abort conversion of file "
        ...
        if (!childID.equals(parentID)) {
          ...
          if (!(id2edu.containsKey(childID))) {
            throw new PepperModuleDataException(this,
                "Input error: relation with missing source element: " + childID + "\n");
          }
          if (!(id2edu.containsKey(parentID))) {
            throw new PepperModuleDataException(this,
                "Input error: relation with missing target element: " + parentID + "\n");
          }
